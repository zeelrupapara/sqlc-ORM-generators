...
const getBookDetailsByID = `-- name: GetBookDetailsByID :one
SELECT
   authors.id, authors.name, authors.bio, books.id, books.name, books.author_id
FROM
    books
JOIN
...
type GetBookDetailsByIDRow struct {
